...
class Bootbase_Widget_Latest_Posts extends WP_Widget {

	function __construct() {
		$widget_ops = array(
			'classname' => 'bootbase_widget_latest_posts',
			'description' => __( 'A widget that displays popular posts from blog', 'bootbase' ),
		);

		$control_ops = array(
			'width' => 300,
			'height' => 350,
			'id_base' => 'bootbase_widget_latest_posts',
		);

		parent::__construct( 'bootbase_widget_latest_posts', __( '[MT] - Latest Posts', 'bootbase-lite' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		while ( $popular_posts->have_posts() ) {
			...
			if ( has_post_thumbnail() ) {
				...
				echo '<div class="bootbase-lite-latest-posts-image" style="float:left; margin-right: 10px;">';
				echo '<img alt=" " title=" " width="' . $attachment[1] . '" height="' . $attachment[2] . '" src="' . $attachment[0] . '">';
				echo '</div>';
			}

			echo '<div class="bootbase-lite-latest-posts-text">';
	...
	function form( $instance ) {
		...
		<div class="ewf-meta">
		  ...
		  <p>
			...
			<label for="<?php echo $this->get_field_id( 'show_title' ); ?>"><?php _e( 'Show widget title?', 'bootbase-lite' ); ?></label>
		  </p>

			<p>
				<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'bootbase-lite' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'items' ); ?>"><?php _e( 'How many post to show:', 'bootbase-lite' ); ?></label>
...
// register the widget
	register_widget( 'bootbase_widget_latest_posts' );
